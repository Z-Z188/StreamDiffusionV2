...
class SingleGPUInferencePipeline:
    ...
    def load_model(self, checkpoint_folder: str):
        ...
        try:
            ...
            self.logger.warning(
                f"Strict load_state_dict failed: {e}; retrying with strict=False")
    ...
    def run_inference(self, input_video_original: torch.Tensor, prompts: list,
        ...
        if input_video_original is not None:
            ...
            noisy_latents = torch.randn(1, 1 + self.pipeline.num_frame_per_block, 16, self.pipeline.height,
                                        self.pipeline.width, device=self.device, dtype=torch.bfloat16)
        ...
        while self.processed < num_chunks + num_steps - 1:
            ...
            current_end = current_end + \
                (chunck_size // 4) * self.pipeline.frame_seq_length  # chunck_size = 4, 完美匹配了VAE的4倍时间压缩率。
            ...
            if input_video_original is not None and end_idx <= input_video_original.shape[2]:
                ...
                noise = torch.randn_like(latents)
                noisy_latents = noise * noise_scale + \
                    latents * (1 - noise_scale)
            ...
            if self.processed >= num_steps:
                ...
                self.logger.info(   # fps_list 里的第一个 fps 确实会偏低，因为它的时间间隔 t 把首帧的初始化开销也算进去了
                    f"Processed {self.processed}, time: {t:.4f} s, FPS: {fps_test:.4f}")
                start_time = end_time
...
def main():
    ...
    parser.add_argument("--checkpoint_folder", type=str,
                        required=True, help="Checkpoint folder path")
    parser.add_argument("--output_folder", type=str,
                        required=True, help="Output folder path")
    parser.add_argument("--prompt_file_path", type=str,
                        required=True, help="Prompt file path")
    ...
    chunck_size = 4
    num_chunks = (t - 1) // chunck_size    # -1为了让 chunk 的计数从 0 开始
